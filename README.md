# tdnet-digest

<img width="542" height="307" alt="image" src="https://github.com/user-attachments/assets/86ada69f-3329-400e-bae9-a00fae210ccb" />

**tdnet-digest** は、TDnet（適時開示情報閲覧サービス）の開示一覧ページに「要約」ボタンを追加し、
開示本文（PDFファイル）を LLM に投げて要約を返す Chrome 拡張です。

- TDnet の適時開示を、ページ上でそのまま AI 要約
- 別タブ遷移やコピペなしで「要点」「ポイントだけ」を即確認
- 利用する LLM / API キー（例: OpenAI 互換 API）はユーザー側で設定
- 情報抽出と整形を分けた2パス要約で、低コストモデルでも安定した表示
- 根拠ページ付きで、短期・中期・長期の見方、好材料、リスク、次回確認点を整理
- 決算、業績修正、株主優待、配当、自己株取得、株式分割、資本政策、M&A、月次・事業進捗、ガバナンス、その他の11分類に対応
- 分析仕様・モデル・抽出方式ごとに要約をローカルキャッシュし、古い分析結果の混在を防止
- 全文抽出（推奨）と要点抽出、2パス要約と1パス要約を設定画面で切り替え可能
- 検証済みの時間軸別方向性を固定換算する「実験的スコア」を任意で表示（既定OFF、2パスのみ）

2パス要約では抽出JSONを検証し、不正な場合は1回だけ修復します。修復できない場合は、生のJSONを表示せず1パス要約へフォールバックします。市場コンセンサス、現在株価、バリュエーションなどPDFにない情報は推測しません。

実験的スコアは、PDFから抽出した短期・中期・長期の方向性を文書タイプ別の固定重みで換算した参考値です。投資判断や将来収益を保証するものではありません。

日本株の決算・IR を毎日追う投資家・トレーダー向けに、
「全文を読む前にざっくり中身と重要度を把握する」ためのツールです。

## 💰 APIコストの目安

> 詳細は [docs/api-cost.md](docs/api-cost.md) を参照してください。

OpenRouter経由での全文抽出モード利用時のコスト目安です（1リクエストあたり入力約56,000トークン、出力約800トークン）。

| モデル                                   | 1回あたり | 100回あたり | 対Gemini比 |
| ---------------------------------------- | --------- | ----------- | ---------- |
| **google/gemini-2.5-flash-lite（推奨）** | **≈ ¥1**  | **≈ ¥90**   | **1.0x**   |
| google/gemini-2.5-flash                  | ≈ ¥3      | ≈ ¥285      | 3.2x       |
| openai/gpt-4o-mini                       | ≈ ¥1.5    | ≈ ¥134      | 1.5x       |

> **Gemini 2.5 Flash Lite** は全文抽出モードでも100回で約¥90と非常に低コストです。2パスモード利用時は約2倍のコストになります。日本円は1ドル=150円で換算しています。

## 📦 インストール方法

### GitHub Releasesからインストール

1. [Releases ページ](https://github.com/ivgtr/tdnet-digest/releases) にアクセス
2. 最新版の **Assets** から `tdnet-digest-vX.Y.Z.zip` をダウンロード
3. 任意の場所に解凍
4. Chrome で `chrome://extensions/` を開く
5. 右上の「**デベロッパーモード**」を **ON** にする
6. 「**パッケージ化されていない拡張機能を読み込む**」をクリック
7. 解凍したフォルダを選択

⚠️ **注意**: 開発者モードでインストールするため、Chrome起動時に警告が表示されますが、正常な動作です。

### 手動ビルド

```bash
# リポジトリをクローン
git clone https://github.com/ivgtr/tdnet-digest.git
cd tdnet-digest

# 依存関係をインストール
npm install

# ビルド
npm run build

# dist/ フォルダをChromeの拡張機能として読み込む
```

## 🚀 開発

### 開発コマンド

```bash
# 開発モード（ファイル監視+自動ビルド）
npm run dev

# プロダクションビルド
npm run build

# 型チェック
npm run type-check

# Lint
npm run lint

# テスト
npm test

# フォーマット
npm run format
```

公開TDnet PDFを使ったローカルLLM評価は、製品機能と分離して`evaluation/`配下に用意しています。APIキーを`.env`に設定する手順は [evaluation/README.md](evaluation/README.md#ローカルllm評価) を参照してください。

## 📄 ライセンス

MIT
