...
**tdnet-digest** は、TDnet（適時開示情報閲覧サービス）の開示一覧ページに「要約」ボタンを追加し、
...
- 情報抽出と整形を分けた2パス要約で、低コストモデルでも安定した表示
- 要約結果をローカルにキャッシュし、再表示時のAPI呼び出しを削減
- 全文抽出（推奨）と要点抽出、2パス要約と1パス要約を設定画面で切り替え可能
...
> 詳細は [docs/api-cost.md](docs/api-cost.md) を参照してください。
...
> **Gemini 2.5 Flash Lite** は全文抽出モードでも100回で約¥90と非常に低コストです。2パスモード利用時は約2倍のコストになります。日本円は1ドル=150円で換算しています。
